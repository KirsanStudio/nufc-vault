...
(async function () {
  ...
  try {
    ...
    if (!upRes.ok) throw new Error(`Upcoming: ${upRes.status}`);
    if (!reRes.ok) throw new Error(`Recent: ${reRes.status}`);
    ...
    console.error(err);
    setStatus("Couldn’t load fixtures. Try refreshing shortly.");
